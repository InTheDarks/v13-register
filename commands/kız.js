...
if (!member || !Age || !Name) return message.reply({ embeds: [embed.setDescription(`:x: **[HATA]** Yanlış Kullanım \`Örnek: .k <@Kullanıcı/ID> Isim Yaş\` `)] }).then(msg => {setTimeout(() => msg.delete(), 5000);});
...
if (member.roles.cache.get(config.kız) || member.roles.cache.get(config.kadın)) return message.reply({ embeds: [embed.setDescription(`Kayıtlı Kullanıcıyı Tekrar Kayıt Edemezsin.`)] }).then(msg => {setTimeout(() => msg.delete(), 5000);});
...
.setColor(config.embedcolor)
.setDescription(`**Bir Üye KIZ Olarak Kaydedildi!**
